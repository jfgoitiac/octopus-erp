...
import { Camera, User, Mail, Phone, IdCard, Save, Loader2, KeyRound, ChevronRight, ShieldCheck } from 'lucide-react';

import { usePortalPerfil } from '../hooks/usePortalPerfil';
import SkeletonCard from '../components/SkeletonCard';

const TAMANO_MAX_MB = 5;

const PortalPerfil = () => {
  const { perfil, loading, guardando, subiendoFoto, guardarPerfil, subirFoto } = usePortalPerfil();
  ...
  const nombreCompleto = `${form.first_name} ${form.last_name}`.trim() || perfil?.username || 'Representante';
        ...
        <div className="relative">
          {fotoMostrada ? (
            <img
              src={fotoMostrada}
              alt={nombreCompleto}
              className="w-24 h-24 rounded-full object-cover ring-4 ring-[var(--portal-primary)]/20"
            />
          ) : (
            <div
              className="w-24 h-24 rounded-full flex items-center justify-center text-2xl font-bold text-white ring-4 ring-[var(--portal-primary)]/20"
