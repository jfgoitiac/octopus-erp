...
import { getMensajesPortal, enviarMensajePortal, marcarMensajeLeidoPortal } from '../api/comunicacion.service';

export function usePortalMensajes(alumnoId) {
  ...
  const fetchMensajes = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getMensajesPortal(alumnoId, controller.signal);
  ...
  const enviar = useCallback(async (cuerpo) => {
    ...
    setEnviando(true);
    try {
      const res = await enviarMensajePortal({ alumno_id: alumnoId, cuerpo });
  ...
  const marcarLeido = useCallback(async (id) => {
    try {
      await marcarMensajeLeidoPortal(id);
