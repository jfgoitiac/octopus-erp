...
function calcMontoNomina(emp, periodo, convenioNomina = 'avec_ve', conceptosUniversales = {}) {
    ...
    if (esDocente) {
        ...
        if (sb > 0) {
            const avec = calcAVEC(sb, emp.anos_servicio, emp.numero_hijos, convenioNomina === 'avec_ve' ? emp.postgrado : emp.titulo, convenioNomina, conceptosUniversales);
            neto = avec.neto;
            ok   = true;
        }
    } else {
        ...
        if (conceptoFor === 'incentivo') {
            ...
            for (const row of sourceRows) {
                ...
                if (esDocente) {
                    ...
                    if (sb > 0) {
                        const avec = calcAVEC(sb, row.anos_servicio, row.numero_hijos, convenioNomina === 'avec_ve' ? row.postgrado : row.titulo, convenioNomina, conceptosUniversales);
