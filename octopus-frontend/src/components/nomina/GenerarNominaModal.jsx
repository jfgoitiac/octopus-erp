...
export function GenerarNominaModal({ onClose, onGenerated }) {
    ...
    const footer = (
        <>
            ...
            <button type="submit" form="form-generar-nomina" disabled={loading || saving}
                className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm text-white disabled:opacity-50"
                style={{ background: 'var(--pb)' }}>
                {saving ? <Loader2 size={15} className="animate-spin" /> : <Calendar size={15} />} Generar
            </button>
        </>
    );
