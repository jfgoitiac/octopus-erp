...
export default function TicketVenta({ venta, onCerrar, onDescargarPdf, descargando }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ticket-venta-titulo"
    >
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-1">
          <h3 id="ticket-venta-titulo" className="font-bold flex items-center gap-2" style={{ color: 'var(--jet)' }}>
            <CheckCircle2 size={18} style={{ color: 'var(--pb)' }} />
            Venta #{venta.id} cobrada
          </h3>
          <button onClick={onCerrar} className="p-1 rounded-lg" style={{ color: 'var(--ash)' }} aria-label="Cerrar">
            <X size={18} />
          </button>
        </div>
