...
export default function ReponerTarjetaModal({ tarjetaInicial, onClose, onRepuesta }) {
  ...
  const handleKeyDown = (e) => { if (e.key === 'Escape' && !reponiendo) onClose(); };
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="modal-reponer-titulo"
      tabIndex={-1}
    >
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-1">
          <h3 id="modal-reponer-titulo" className="font-bold flex items-center gap-2" style={{ color: 'var(--jet)' }}>
            <ShieldAlert size={18} style={{ color: '#b45309' }} />
            Reponer tarjeta extraviada/dañada
          </h3>
          <button
            onClick={() => !reponiendo && onClose()}
